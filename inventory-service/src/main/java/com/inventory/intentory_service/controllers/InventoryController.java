...
public class InventoryController {
  ...
  @GetMapping
  public ResponseEntity<Map<String, Object>> getAll(@PageableDefault(size = 10) Pageable pageable) {
  ...
  public ResponseEntity<InventoryResponse> purchase(@Valid @RequestBody PurchaseRequest request) {
    return new ResponseEntity<>(
        inventoryService.purchase(request.getProductId(), request.getQuantity()),
        HttpStatus.CREATED);
  }
